  ...
  for(var i in this.games) {
    if(this.games[i].length >= 3) {
      ...
      for(var j in this.games[i]) {
        if(j == 0 || typeof this.games[i] === 'undefined')
          continue;
          ...
          if(player.dead) {
            ...
            for(var k in game) {
              if(k == 0)
                continue;
              var opPoints = 0;
              game[k].get('player', function(err, gPlayer) {
                console.log({yourPoints: gPlayer.points, opPoints: self.getOpPoints(gPlayer)})
                game[k].emit('gameover', {yourPoints: gPlayer.points, opPoints: self.getOpPoints(gPlayer)});
              });
            }
            self.games.splice(player.gameId, 1);
          } else {
...
Game.prototype.getOpPoints = function(player) {
  var game = this.games[player.gameId];
  var ret = 0;
  for(var i in game) {
    if(i == 0)
      continue;
    game[i].get('player', function(err, gPlayer) {
      if(player != gPlayer) {
        ret = gPlayer.points;
      }
    });
