...
Game.prototype.generateFood = function(gameId, foodIds) {
  ...
  for(var i in foodIds) {
    ...
    var ret = [];
      ...
      if(!init) {
        for(var j in game[0]) {
          var sLen = game[0][j].length - 1;
          if(game[0][j][sLen][0] == ret[0] && game[0][j][sLen][1] == ret[1])
            ret = [];
        }
      }
    }
...
Game.prototype.checkShift = function(player) {
  var food = this.games[player.gameId][0];
  var ret = true;
  for(var i in food) {
    for(var j in food[i]) {
      if(player.snake[0][0] == food[i][j][0] && player.snake[0][1] == food[i][j][1]) {
        ret = false;
        food[i][j].shift();
      }
    }
  ...
  for(var i in this.games) {
    if(this.games[i].length >= 3) {
      ...
      for(var j in this.games[i]) {
        ...
        sockets.push(socket);
        socket.get('player', function(err, player) {
          player.shift = self.checkShift(player);
          self.update(player);
          socket.set('player', player);
